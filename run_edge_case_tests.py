"""
Run insurance-whittaker edge-case tests on Databricks via the Jobs API (serverless).

Uploads all source + all test files including the new test_edge_cases.py.
"""
...
src_files = {
    ...
    "py.typed": base64.b64encode(b"").decode("utf-8"),
}

test_files = {
    "test_edge_cases.py": read_b64(f"{base}/tests/test_edge_cases.py"),
    # Also run existing tests to verify we haven't broken anything
    ...
    "test_plots.py": read_b64(f"{base}/tests/test_plots.py"),
}
...
lines = [
    ...
    "    'numpy', 'scipy', 'polars', 'pytest', 'matplotlib', '--quiet'], check=True)",
    "",
...
lines.extend([
    ...
    "",
    "# Run pytest on edge-case tests first, then all uploaded tests",
    "env = dict(os.environ)",
...
notebook_path = "/Workspace/Shared/insurance-whittaker-edge-cases"
...
run_waiter = w.jobs.submit(
    run_name="insurance-whittaker-edge-cases",
    tasks=[
        jobs.SubmitTask(
            task_key="run-tests",
